...
| [#23](https://github.com/byoridb/byoridb/issues/23) | Engine half resolved; issue open for the client half | The server maps missing or expired HTTP sessions to `401 SESSION_EXPIRED` ([PR #35](https://github.com/byoridb/byoridb/pull/35)), and an authorization denial is no longer carried by the same status: it is `403 PERMISSION_DENIED` ([PR #85](https://github.com/byoridb/byoridb/pull/85)), so `401` means exactly "a new session will help". v0.4.0 ships both. Remaining: `byori`'s client and MCP must drop the `400`-plus-substring heuristic for status codes, add a `429`/`Retry-After` branch for the login throttle ([PR #91](https://github.com/byoridb/byoridb/pull/91)), and cover it with an integration test. |
| [#24](https://github.com/byoridb/byoridb/issues/24) | Partial; issue open | New tagged releases build Linux x86_64/arm64 and macOS x86_64/arm64 archives, and every macOS binary is now Developer ID signed under the hardened runtime and notarized, with the release failing on anything other than `Accepted`. The ticket is not stapled, because `stapler` cannot write into a bare executable, so Gatekeeper resolves it online. Windows remains unsupported, and v0.3.3 and earlier archives are not signed retroactively. |
