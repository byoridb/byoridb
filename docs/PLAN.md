...
| [#121](https://github.com/byoridb/byoridb/issues/121) | Resolved in code; issue open pending review | `GO` deduplicated rows by destination VID alone, so a second edge to the same vertex — another type under `OVER a,b`, or another rank — was silently dropped while the query reported success; `FETCH`, `MATCH`, and `LOOKUP` all disagreed with it. Row emission is now keyed on `(dst, edge_type, rank)` and frontier expansion stays keyed on the destination, so cycles are still bounded. The key was deliberately narrowed rather than widened to the edge's full identity: `GO` reports a destination reached by several same-type paths once, which `test_go_traversal_deduplicates_destinations_per_source` covers, and that behaviour is preserved. A same-type same-rank convergence therefore still collapses, which is pre-existing and unchanged. |
| [#124](https://github.com/byoridb/byoridb/issues/124) | Resolved in code; issue open pending review | Node patterns accept `|` between labels, and the separator now carries meaning: `(n:A:B)` requires every listed tag, `(n:A|B)` at least one. This began as an enhancement and turned out to fix a silent defect — the matcher accepted "at least one" while candidate selection used only `labels.first()`, so `(n:A:B)` meant `(n:A)` and swapping the order changed the answer. The asymmetry with edge patterns (where a repeated colon is a synonym for union) is deliberate and documented: an edge has exactly one type, a vertex holds a set of tags. Mixing separators in one pattern is refused rather than given an arbitrary precedence. A `Any` union over several labels skips the single-label tag index, because indexing on the first label would return a subset rather than the superset the matcher can narrow. |
| [#122](https://github.com/byoridb/byoridb/issues/122) | Resolved in code; issue open pending review | `scan_edge_index` honoured only `limit` and ignored `ScanOptions` bounds, so there was no bounded form for a `lookup_edge_range` to build on and ordered predicates on an indexed edge property took a scan. It now applies bounds the way `scan_tag_index` does, and `lookup_edge_range` mirrors `lookup_tag_range` including its guard that only bool, int, and non-zero non-NaN float boundaries are ordered — the string encoding is length-prefixed and not lexical. Wired into `execute_lookup_edge` ahead of the equality path and into `EXPLAIN`, which now reports `IndexScan` for an ordered edge range; distributed mode refuses it as the tag path does. Verified at the storage layer, because the executor revalidates every candidate against the stored edge — an unbounded scan there still returns the right answer, so only a storage-level test can tell whether the range was applied. |
| [#105](https://github.com/byoridb/byoridb/issues/105) | Decided: stay on bincode 1.3.3; issue closed | The encoder is pinned deliberately. RUSTSEC-2025-0141 marks bincode unmaintained, but it is informational and the CI audit action does not fail on informational advisories. bincode encodes redb values, the Raft log, snapshots, and peer RPC payloads, so replacing it changes what deployments already hold on disk and what peers exchange; 2.x/3.x additionally removed the serde free functions in use and carry the same advisory. Revisit when a storage-format change is planned for other reasons so the migration is paid once. Recorded under Known constraints → Operations, with the `KVStoreError` encoder-type leak named as the prerequisite worth doing first. |
| [#125](https://github.com/byoridb/byoridb/issues/125) | Resolved in code; issue open pending review | `LookupType::Edge` was unreachable: the parser always emitted `Tag`. Rather than delete the variant, `LOOKUP ON EDGE <name>` now produces it — because a space may hold a tag **and** an edge type of the same name (verified: both `CREATE` statements succeed and both appear in `SHOW`), and the bare form resolves such a name to the tag, leaving the edge unreachable and a predicate over its properties silently empty. No `LOOKUP ON TAG` was added: without a third "unspecified" state it would behave exactly like the bare form, including its fall back to an edge, which an explicit spelling should not do. The tag-first precedence is now documented. |
| [#123](https://github.com/byoridb/byoridb/issues/123) | Resolved in code; issue open pending review | `UPDATE EDGE` moved its secondary index entries in separate calls *after* its `batch_apply`, leaving three windows in which a crash could leave the index pointing at the old value or holding neither entry, with no repair path short of rebuilding the index. The keys are now built with `KeyUtils::edge_index_key` and passed as puts and deletes in the same transaction as the edge, matching `INSERT EDGE` and `DELETE EDGE` ([PR #113](https://github.com/byoridb/byoridb/pull/113)); the unchanged-value short-circuit is kept. Introduced by my own [PR #107](https://github.com/byoridb/byoridb/pull/107), before #113 established the in-batch pattern. |
| [#78](https://github.com/byoridb/byoridb/issues/78) | Resolved in code; issue open pending review | Edge patterns accept `|` between types — `[e:t1|t2]` — which is what nGQL, Cypher, and portable tooling emit. A single `|` was not a lexer token at all (only `||`, as `OR`), so those queries failed with a misleading "Unexpected end of input"; `Pipe` is declared after `||` so the longer match still wins and `OR` is unaffected. The repeated-colon form `[e:t1:t2]` still parses, because it was the only spelling that worked and queries may rely on it, and both are now documented with `|` as the one to prefer. Union semantics were already implemented in `algo.rs` (non-empty set = membership filter, empty = every type) but undocumented and untested; an integration test now asserts the alternation result equals the union of the same query run per single type, rather than a hardcoded list. |
| [#79](https://github.com/byoridb/byoridb/issues/79) | Resolved in code; issue open pending review | `LOOKUP ON <edge type>` used to be refused outright. It now resolves tag-vs-edge against the schema — the parser cannot, so it always emits `LookupType::Tag` — and runs a single-field equality through `list_edge_indexes`/`lookup_edge`, revalidating every candidate against the stored edge, or a predicate scan over `{space}:edge:` otherwise. Projects `src`, `dst`, and `rank`, since rank is part of an edge's identity (#108). `EXPLAIN`/`PROFILE` now report `IndexScan` with the index name or `EdgeScan` with a full-scan warning instead of describing an edge LOOKUP as a tag scan; the tag-vs-edge rule is shared with the executor rather than duplicated. **Not** implemented: the bounded-range edge index, because `scan_edge_index` honours only `limit` and not `ScanOptions` bounds, unlike its tag counterpart — ordered predicates fall back to the scan, which is correct but unindexed, and that is the remaining half of the issue. Prerequisite landed separately as [PR #113](https://github.com/byoridb/byoridb/pull/113): `DELETE EDGE` did not retract index entries, so revalidation would have been covering for a missing delete. |
| [#82](https://github.com/byoridb/byoridb/issues/82) | Resolved in code; issue open pending review | `spaces.md` and its mirror gain a "Migrating from client-hashed integer VIDs" section answering the five questions the issue listed, each established by `fixed_string_vid_migration_path_from_client_hashed_int64` rather than asserted from the design. The decisive fact the issue did not have: **`vid_type` is immutable** — `ALTER` has only `TAG`/`EDGE`/`USER` forms and the type comes from the `CREATE SPACE` descriptor — so a client-hashed `INT64` space is migrated by copying into a new `FIXED_STRING` space, and the legacy non-negative bridge does not apply to it at all (that bridge is for a space whose descriptor already says `FIXED_STRING`). History is keyed by space and VID, so a re-key starts a new history and the old space stays the archive of record; there is no supported way to carry history across it. Writes reject every integer VID with the read/delete-only message, while the surrogate-specific refusal is reachable only on read/delete. |
| [#110](https://github.com/byoridb/byoridb/issues/110) | Resolved in code; issue open pending review | `parse()` now requires end of input after a complete statement, erroring with the unexpected token's kind and location instead of discarding it. Previously any trailing input was dropped silently, so an unsupported clause looked supported and a truncated statement ran its prefix while reporting success — that is what made #108 invisible. **Behaviour change:** input that used to be accepted now fails. Measured before landing: the 995-test suite passes unchanged, `byoridb/byori`'s MCP client sends one complete statement per request (its read-only path already refuses `;`) and all of its generated statements parse, and every tolerance still parses — trailing `;`, `//` and `/* */` comments leading, trailing, or mid-statement, compound statements, and `$var` assignment. Error messages redact literal payloads, since a trailing token can be one. Also corrected `ngql-syntax.md`, which documented `--` as a line comment: the lexer only accepts `//` and `/* */`, so a trailing `--` comment had been "working" purely because the junk after it was ignored. |
| [#108](https://github.com/byoridb/byoridb/issues/108) | Resolved in code; issue open pending review | `FETCH PROP ON <edge> src->dst@rank` now addresses exactly that edge. The parser never read `@rank` on this path, and `parse()` does not require end-of-input after a statement, so the suffix was silently discarded and the fetch prefix-scanned every rank of the pair. Endpoints moved out of `FetchStatement::vids` — where two flattened expressions had nowhere to put a rank — into `edge_refs: Vec<EdgeRef>`, reusing the type `DELETE EDGE` already had. An omitted rank deliberately keeps matching every rank, unlike the write statements where it means `0`: a fetch without a rank is a query over the pair, and changing it would break existing readers. Documented in `dql.md` and its mirror, which had never said what an omitted rank matched. |
...
- `UPDATE EDGE` updates an existing edge's properties, refreshing the reverse
  ...
  no-op rather than a creation. Edge `LOOKUP` resolves a single-field equality
  through the edge index and otherwise takes a predicate scan; the edge index has
  no bounded-range form, so ordered predicates on an edge are correct but
  unindexed. Local single-field Boolean and integer ordered predicates can use a tag index; strings,
...
- **The serialization encoder is deliberately pinned to `bincode` 1.3.3, which
  RustSec flags as unmaintained (RUSTSEC-2025-0141).** The advisory is
  informational, and the CI audit action reports informational advisories without
  failing, so nothing is blocked. Staying is a decision rather than an oversight
  ([#105](https://github.com/byoridb/byoridb/issues/105)): bincode encodes the
  redb values, the Raft log, snapshots, and peer RPC payloads, so changing the
  encoder changes what existing deployments already hold on disk and what peers
  exchange. bincode 2.x/3.x also removed the serde free functions in use and
  would land on a crate carrying the same advisory. Revisit when a storage-format
  change is planned for other reasons, so the migration is paid once; a
  prerequisite worth doing first either way is to stop exposing the encoder's
  error type through `KVStoreError`, so a future swap is not itself a breaking
  change.
