...
| [#121](https://github.com/byoridb/byoridb/issues/121) | 코드 해결·리뷰 대기로 이슈 열림 | `GO`가 행을 목적지 VID만으로 dedupe해서, 같은 정점으로 가는 두 번째 edge(`OVER a,b`의 다른 타입, 또는 다른 rank)가 조용히 버려지면서 쿼리는 성공을 보고했다. `FETCH`·`MATCH`·`LOOKUP` 모두 `GO`와 불일치했다. 이제 행 생성은 `(dst, edge_type, rank)`로 키잉하고 프론티어 확장은 목적지 기준을 유지해 사이클이 여전히 제한된다. 키를 edge 전체 신원으로 넓히지 않고 **좁힌** 것은 의도적이다 — `GO`는 여러 같은-타입 경로로 도달한 목적지를 한 번만 보고하며 `test_go_traversal_deduplicates_destinations_per_source`가 그걸 지키고, 그 동작을 보존했다. 따라서 같은 타입·같은 rank로 수렴하는 경우는 여전히 합쳐지며 이는 기존 동작 그대로다. |
| [#124](https://github.com/byoridb/byoridb/issues/124) | 코드 해결·리뷰 대기로 이슈 열림 | node 패턴이 라벨 사이의 `|`를 받고, 구분자가 의미를 갖게 했다 — `(n:A:B)`는 나열한 tag를 모두, `(n:A|B)`는 하나 이상을 요구한다. enhancement로 시작했지만 실제로는 조용한 결함을 고친 것이다 — matcher는 "하나 이상"을 받아들이는데 후보 선택이 `labels.first()`만 써서 `(n:A:B)`가 `(n:A)`를 의미했고 순서를 바꾸면 답이 달라졌다. edge 패턴(콜론 반복이 합집합의 동의어)과의 비대칭은 의도적이고 문서화했다 — edge는 타입이 정확히 하나, 정점은 tag의 집합을 갖는다. 한 패턴에서 구분자를 섞으면 임의 우선순위를 주지 않고 거부한다. 여러 라벨의 `Any` 합집합은 단일 라벨 tag 인덱스를 건너뛴다 — 첫 라벨로 인덱싱하면 matcher가 좁힐 수 있는 superset이 아니라 subset이 되기 때문이다. |
| [#122](https://github.com/byoridb/byoridb/issues/122) | 코드 해결·리뷰 대기로 이슈 열림 | `scan_edge_index`가 `limit`만 보고 `ScanOptions` 경계를 무시해서 `lookup_edge_range`를 만들 바탕이 없었고, 인덱스가 있는 edge 속성의 순서 비교가 스캔으로 떨어졌다. 이제 `scan_tag_index`와 같은 방식으로 경계를 적용하고, `lookup_edge_range`가 `lookup_tag_range`를 미러링한다 — bool·정수·0이 아닌 non-NaN float만 순서가 있다는 가드 포함(문자열 인코딩은 length-prefixed라 lexical하지 않다). `execute_lookup_edge`의 동등 조건 경로 앞에 연결하고 `EXPLAIN`도 순서 비교에 `IndexScan`을 보고하게 했으며, 분산 모드는 tag 경로와 동일하게 거부한다. 검증은 스토리지 계층에서 했다 — executor가 모든 후보를 저장된 edge와 대조해 재검증하므로 경계가 무시돼도 결과는 맞고, 경계 적용 여부는 스토리지 레벨 테스트만 구별할 수 있다. |
| [#105](https://github.com/byoridb/byoridb/issues/105) | 결정: bincode 1.3.3 유지·이슈 닫힘 | 인코더를 의도적으로 고정했다. RUSTSEC-2025-0141이 bincode를 unmaintained로 표기하지만 informational이고 CI 감사 액션은 informational 권고로 실패하지 않는다. bincode는 redb 값·Raft 로그·스냅샷·피어 RPC payload를 인코딩하므로 교체하면 배포본이 이미 디스크에 들고 있는 것과 피어 간 교환 포맷이 바뀐다. 게다가 2.x/3.x는 사용 중인 serde 자유 함수를 제거했고 같은 권고를 그대로 안고 있다. 다른 이유로 저장 포맷을 바꿀 계획이 생길 때 함께 치러 마이그레이션을 한 번만 하도록 재검토한다. Known constraints → Operations에 기록했고, 선결 과제로 `KVStoreError`가 인코더 에러 타입을 노출하는 문제를 지목했다. |
| [#125](https://github.com/byoridb/byoridb/issues/125) | 코드 해결·리뷰 대기로 이슈 열림 | `LookupType::Edge`가 도달 불가였다 — 파서가 항상 `Tag`를 냈다. 변형을 지우는 대신 `LOOKUP ON EDGE <name>`이 그것을 만들게 했다. 한 space에 같은 이름의 tag와 edge type이 함께 있을 수 있고(실증: 두 `CREATE` 모두 성공하고 `SHOW`에 둘 다 나옴), 이름만 쓰면 tag로 해석되어 그 edge에 도달할 방법이 없고 edge 속성 조건이 조용히 0건이 되기 때문이다. `LOOKUP ON TAG`는 추가하지 않았다 — 세 번째 "미지정" 상태가 없으면 이름만 쓰는 형태와 완전히 같게 동작하고 edge로 fallback까지 하므로, 명시적 표기가 해서는 안 되는 일이다. tag 우선 규칙을 문서화했다. |
| [#123](https://github.com/byoridb/byoridb/issues/123) | 코드 해결·리뷰 대기로 이슈 열림 | `UPDATE EDGE`가 secondary index 항목을 `batch_apply` **이후** 별도 호출로 옮기고 있었고, 그 사이 크래시가 나면 인덱스가 이전 값을 가리키거나 두 항목 모두 없는 상태가 되며 인덱스 재생성 외에 복구 경로가 없었다. 이제 `KeyUtils::edge_index_key`로 키를 만들어 edge와 같은 트랜잭션의 put·delete로 넘긴다 — `INSERT EDGE`·`DELETE EDGE`([PR #113](https://github.com/byoridb/byoridb/pull/113))와 동일한 방식이며, 값이 안 바뀐 경우의 short-circuit은 유지했다. 원인은 내가 만든 [PR #107](https://github.com/byoridb/byoridb/pull/107)이고, #113이 배치 내 처리 패턴을 세우기 전이었다. |
| [#78](https://github.com/byoridb/byoridb/issues/78) | 코드 해결·리뷰 대기로 이슈 열림 | edge 패턴이 타입 사이의 `|`를 받는다 — `[e:t1|t2]` — nGQL·Cypher·이식 가능한 도구가 내보내는 표기다. 단일 `|`는 렉서 토큰이 아예 없었고(`||`만 `OR`로 존재) 그래서 그런 쿼리가 "Unexpected end of input"이라는 오해를 유발하는 오류로 실패했다. `Pipe`를 `||` 뒤에 선언해 긴 매치가 계속 이기므로 `OR`는 영향받지 않는다. 콜론 반복 `[e:t1:t2]`도 여전히 파싱된다 — 유일하게 동작하던 표기라 쿼리가 의존할 수 있기 때문이고, 이제 둘 다 문서화하되 `|`를 권장한다. 합집합 의미론은 `algo.rs`에 이미 구현돼 있었지만(비어 있지 않은 집합 = membership 필터, 빈 집합 = 모든 타입) 문서도 테스트도 없었다. 통합 테스트가 alternation 결과를 하드코딩한 목록이 아니라 단일 타입별 질의 결과의 합집합과 비교해 검증한다. |
| [#79](https://github.com/byoridb/byoridb/issues/79) | 코드 해결·리뷰 대기로 이슈 열림 | `LOOKUP ON <edge type>`이 이전에는 즉시 거부됐다. 이제 tag/edge를 schema로 판별하고(파서는 판별할 수 없어 항상 `LookupType::Tag`를 냄), 단일 필드 동등 조건은 `list_edge_indexes`/`lookup_edge`로 처리하며 모든 후보를 저장된 edge와 대조해 재검증한다. 그 외에는 `{space}:edge:` 술어 스캔. rank가 edge identity의 일부(#108)이므로 `src`·`dst`·`rank`를 투영한다. `EXPLAIN`/`PROFILE`도 edge LOOKUP을 tag scan으로 서술하지 않고 인덱스명과 함께 `IndexScan`, 또는 full-scan 경고와 함께 `EdgeScan`으로 보고하며, tag/edge 판별 규칙은 executor와 공유해 중복하지 않는다. **미구현**: bounded-range edge index — `scan_edge_index`가 tag 쪽과 달리 `ScanOptions` 경계를 무시하고 `limit`만 본다. 따라서 순서 비교 술어는 스캔으로 fallback하며(정확하지만 index 미사용) 그것이 이슈의 남은 절반이다. 선결 수정은 [PR #113](https://github.com/byoridb/byoridb/pull/113)으로 분리했다 — `DELETE EDGE`가 인덱스 항목을 지우지 않아, 재검증이 누락된 삭제를 가리는 역할이 될 뻔했다. |
| [#82](https://github.com/byoridb/byoridb/issues/82) | 코드 해결·리뷰 대기로 이슈 열림 | `spaces.md`와 미러에 "클라이언트가 해싱한 정수 VID에서 마이그레이션" 절을 추가해 이슈가 열거한 다섯 질문에 답했고, 각 답은 설계 추론이 아니라 `fixed_string_vid_migration_path_from_client_hashed_int64`로 실증했다. 이슈가 갖고 있지 않던 결정적 사실: **`vid_type`은 불변이다** — `ALTER`에는 `TAG`/`EDGE`/`USER`만 있고 타입은 `CREATE SPACE` descriptor에서 온다 — 따라서 클라이언트가 해싱한 `INT64` space는 새 `FIXED_STRING` space로 복사해 마이그레이션하며, legacy 0 이상 bridge는 여기에 아예 적용되지 않는다(그 bridge는 descriptor가 이미 `FIXED_STRING`인 space용). 이력은 space와 VID로 키잉되므로 재키는 새 이력을 시작하고 이전 space가 기록 보관소로 남는다 — 이력을 넘겨오는 지원 방법은 없다. 쓰기는 모든 정수 VID를 read/delete-only 메시지로 거부하고, surrogate 전용 거부는 읽기·삭제에서만 도달한다. |
| [#110](https://github.com/byoridb/byoridb/issues/110) | 코드 해결·리뷰 대기로 이슈 열림 | `parse()`가 완결된 문장 뒤 입력 종료를 요구하고, 남은 토큰을 버리지 않고 종류·위치를 명시한 오류를 낸다. 이전에는 남은 입력을 조용히 버려서 미지원 절이 지원되는 것처럼 보이고 잘린 문장이 앞부분만 실행하며 성공을 보고했다 — #108이 보이지 않았던 이유다. **동작 변경:** 지금까지 통과했던 입력이 실패한다. 착륙 전 실측: 995개 테스트가 그대로 통과하고, `byoridb/byori`의 MCP 클라이언트는 요청당 완결된 문장 하나만 보내며(read-only 경로는 이미 `;`를 거부) 생성하는 모든 문장이 파싱되고, 관용 케이스도 전부 유지된다 — 끝의 `;`, 문장 앞·뒤·중간의 `//`와 `/* */` 주석, compound, `$var` 할당. trailing 토큰이 리터럴일 수 있으므로 오류 메시지는 payload를 가린다. `--`를 줄 주석으로 적어둔 `ngql-syntax.md`도 정정했다 — 렉서는 `//`와 `/* */`만 받으므로, 끝의 `--` 주석은 그 뒤 쓰레기가 무시된 덕에 "동작하는 것처럼" 보였을 뿐이다. |
| [#108](https://github.com/byoridb/byoridb/issues/108) | 코드 해결·리뷰 대기로 이슈 열림 | `FETCH PROP ON <edge> src->dst@rank`가 이제 그 edge 하나만 지목한다. 이 경로의 파서가 `@rank`를 아예 읽지 않았고 `parse()`가 문장 뒤 입력 종료를 요구하지 않으므로 suffix가 조용히 버려지고 pair의 모든 rank를 prefix scan했다. endpoint를 `FetchStatement::vids`(평탄화된 두 표현식이라 rank를 둘 자리가 없었다)에서 `edge_refs: Vec<EdgeRef>`로 옮겨 `DELETE EDGE`가 이미 쓰던 타입을 재사용했다. rank 생략은 의도적으로 모든 rank 매치를 유지한다 — 쓰기 문장에서 생략이 `0`을 의미하는 것과 다르며, rank 없는 fetch는 pair에 대한 조회이고 바꾸면 기존 reader가 깨진다. rank 생략이 무엇에 매치되는지 한 번도 적혀 있지 않던 `dql.md`와 미러에 문서화했다. |
| [#77](https://github.com/byoridb/byoridb/issues/77) | 코드 해결·리뷰 대기로 이슈 열림 | `UPDATE EDGE`에 실행 경로가 생겼다. 이전에는 planner가 `update_type`·`dst_vid`·`edge_name`·`ranking`을 버려서 tag 없는 vertex 갱신이 되고 "Tag name required for UPDATE"를 냈다 — 문장이 언급조차 하지 않는 개념을 지목하는 오류였다. `UpdatePlan`이 `EdgeUpdateTarget`을 싣고, `execute_update_edge`가 property를 병합해 정방향·역방향 현재뷰 키와 이력 버전을 하나의 `batch_apply`로 쓰고 edge index를 이전 값에서 옮긴다. vertex UPDATE와 달리 **upsert가 아니다** — 없는 edge는 no-op이다. 여기서 만들면 `INSERT EDGE`가 유지하는 차수 counter와 ontology triple까지 유지해야 하고, 둘 중 하나라도 빠지면 미리 계산된 `COUNT`가 깨지거나 추론이 낡는다. source·destination·type·rank는 assignment 대상이 아니므로 갱신이 edge를 이동시키지 않고, 따라서 그 counter들은 구조적으로 불변이다. |
| [#80](https://github.com/byoridb/byoridb/issues/80) | PR #103으로 해결·이슈 닫힘 | `auth.session_ttl_secs` / `BYORIDB__AUTH__SESSION_TTL_SECS`로 bearer session TTL을 설정하며, 기본값은 기존 하드코딩 값인 24시간. 0과 1년 초과는 시작 시 거부. 이슈가 제안한 `[server]` 키가 아니라 `[auth]`에 둬서 #76이 추가한 다른 인증 설정과 함께 있게 했고, 제안된 이름은 릴리스된 적이 없어 의존하는 것이 없음. 이슈의 secondary finding은 실제 문제였음 — `GraphService::with_auth`가 graph session 저장소를 독립적인 24시간 기본값으로 만들고 있었고, 두 저장소가 접근마다 재조정되며 짧은 쪽이 이기므로 24시간을 넘는 TTL 설정은 조용히 깎였을 것. 이제 `with_auth`가 bearer의 TTL에서 graph 저장소 TTL을 파생시키고, `with_session_ttl`은 테스트에서 불일치를 의도적으로 만들 때만 남겨둠 |
