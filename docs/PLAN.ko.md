...
| [#23](https://github.com/byoridb/byoridb/issues/23) | 엔진 절반 해결·클라이언트 절반으로 이슈 열림 | 서버의 만료/누락 세션은 [PR #35](https://github.com/byoridb/byoridb/pull/35)에서 `401 SESSION_EXPIRED`가 됐고, 인가 거부는 더 이상 같은 status를 쓰지 않고 `403 PERMISSION_DENIED`가 되어([PR #85](https://github.com/byoridb/byoridb/pull/85)) `401`은 "새 세션이 도움이 됨"만 의미함. v0.4.0에 둘 다 포함됨. 남은 작업: `byori`의 클라이언트·MCP가 `400`+문자열 휴리스틱을 상태 코드로 대체하고, login throttle의 `429`/`Retry-After` 분기를 추가하고([PR #91](https://github.com/byoridb/byoridb/pull/91)), 통합 테스트로 덮는 것 |
| [#24](https://github.com/byoridb/byoridb/issues/24) | 부분 해결·이슈 열림 | 새 tag release가 Linux x86_64/arm64와 macOS x86_64/arm64 archive를 빌드하고, 모든 macOS binary를 hardened runtime 아래에서 Developer ID로 서명한 뒤 notarization하며 `Accepted`가 아니면 release가 실패함. `stapler`가 단독 실행 파일에 ticket을 쓸 수 없어 staple하지 않으므로 Gatekeeper가 온라인으로 조회함. Windows는 여전히 미지원이고, v0.3.3 이하 archive는 소급 서명하지 않음 |
