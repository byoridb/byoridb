...
a non-empty value. It does not currently enforce password strength beyond that
startup check.
...
- User and role mutation, `BALANCE`, `SHOW USERS`, `SHOW ROLES`, and
  `SHOW SESSIONS` require GOD or ADMIN even when another role has `Create`.
  `SHOW USERS` lists the built-in root account and persisted users without
  password hashes. `SHOW ROLES` currently returns that same user/role listing.
  `SHOW SESSIONS` lists active users and selected spaces but omits bearer
  session IDs. Singular `SHOW USER` and `SHOW ROLE` remain accepted aliases.
...
Session IDs are cryptographically random positive 63-bit values. HTTP represents them as
...
- session IDs;
- `X-ByoriDB-Session-Id` headers;
- authentication request bodies;
- queries containing `PASSWORD`.

ByoriDB's query logs and active-query diagnostics retain bounded metadata rather
than raw query text or session IDs. Reverse proxies and ingress logs still
require separate configuration to exclude sensitive headers and bodies.
...
| `DELETE /api/v1/session` | Live session in `X-ByoriDB-Session-Id`; the session signs itself out |
| `GET /api/v1/diagnostics/queries` | Live `X-ByoriDB-Session-Id` from GOD/ADMIN |
| `GET /health`, `GET /ready` | Public health signals |
| `GET /metrics`, `GET /api/v1/metrics` | Public; protect at the network/proxy layer |

The diagnostics response includes only bounded query metadata. It omits raw
query text and bearer session IDs.
