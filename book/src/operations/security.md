...
A caller may set `read_only` on a single query request. The server then refuses
the request unless every clause it would execute is a read, and refuses
administrative statements as well. See
[the API reference](../reference/api.md) for the request shape.

This exists for one situation: running a statement the caller did not write — one
supplied by an end user or generated by a model — over a session that has write
permission, without granting that statement the session's authority. Enforcing it
in the server means callers no longer reimplement statement classification
outside the parser, where it has to track this grammar and silently permits any
statement type added later.

What it is not:

- **Not a credential or role.** It constrains one request. The same session may
  write on its next request, so it is not a way to hand out limited access.
- **Not a tenant boundary.** Roles still apply to `space="*"`, so a read-only
  request reads every space its session could read.
- **Not a sandbox.** It bounds what a statement can do to the database. It does
  not bound what the calling process can do, and it does not limit how much data
  a read may return.
