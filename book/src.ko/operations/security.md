...
호출자는 개별 query 요청에 `read_only`를 설정할 수 있습니다. 그러면 server는 실행될
모든 절이 읽기가 아니면 요청을 거부하고, 관리 문장도 함께 거부합니다. 요청 형태는
[API 레퍼런스](../reference/api.md)를 참고하세요.

이것은 한 가지 상황을 위한 것입니다 — **호출자가 직접 작성하지 않은 문장**(최종
사용자가 넣거나 모델이 생성한 문장)을 쓰기 권한이 있는 session으로 실행하면서 그
문장에 session의 권한을 주지 않는 것. server가 집행하므로 호출자가 parser 밖에서
문장 분류를 재구현할 필요가 없습니다. 그런 재구현은 이 문법을 계속 따라가야 하고,
나중에 추가된 문장 유형을 조용히 허용합니다.

다음은 아닙니다:

- **credential이나 role이 아닙니다.** 요청 하나만 제약합니다. 같은 session이 다음
  요청에서 쓸 수 있으므로, 제한된 접근 권한을 나눠주는 수단이 아닙니다.
- **tenant 경계가 아닙니다.** role은 여전히 `space="*"`에 적용되므로, 읽기 전용
  요청도 해당 session이 읽을 수 있는 모든 space를 읽습니다.
- **sandbox가 아닙니다.** 문장이 database에 무엇을 할 수 있는지를 제한할 뿐,
  호출 프로세스가 무엇을 할 수 있는지나 읽기가 반환하는 데이터 양은 제한하지 않습니다.
