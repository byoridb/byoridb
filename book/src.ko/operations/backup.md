...
[English](../../operations/backup.html) | **한국어**
...
현재 도구는 실행 중인 server가 연 redb 파일을 동시에 열 수 없습니다. online backup으로
간주하지 마세요. write traffic을 중단하고 `byoridb-server`를 정상 종료한 뒤 backup을
실행하고, 완료 후 server를 다시 시작해야 합니다.
...
snapshot에서 두 테이블을 복사하므로 백업 내부는 한 시점으로 일관됩니다. 이 snapshot
일관성은 실행 중 server와 파일 lock을 공유할 수 있다는 뜻이 아닙니다.
...
현재 `verify`는 backup을 catalog에서 찾을 수 있는지만 확인합니다. `data.redb`를 열거나
table, application record, row count를 검사하지 않습니다. 성공 결과만으로 usable
snapshot이라고 판단하지 말고, 격리된 디렉터리에 정기적으로 복원한 뒤 server를 열어
current/`AS OF` query를 실행하세요.
