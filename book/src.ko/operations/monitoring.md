...
스탠드얼론 서버는 Prometheus text metric, HTTP health signal, 인증된 active-query
diagnostics와 Rust `tracing` log를 제공합니다.

## HTTP endpoint

| Endpoint | 접근 | 현재 동작 |
|---|---|---|
| `GET /health` | 인증 없음 | handler에 도달하면 `200 OK`와 `OK` 반환 |
| `GET /ready` | 인증 없음 | query 수락 중 `200 READY`, graceful shutdown 시작 후 `503 SHUTTING DOWN` |
| `GET /metrics` | 인증 없음 | Prometheus text exposition |
| `GET /api/v1/metrics` | 인증 없음 | `/metrics`를 안내하는 JSON 응답 |
| `GET /api/v1/diagnostics/queries` | `X-ByoriDB-Session-Id`의 GOD/ADMIN session | 민감한 필드를 제외한 active-query metadata |

`/health`는 redb read/write를 확인하는 deep health check가 아닙니다. `/ready`는 새
query 수락 여부를 나타내지만 모든 distributed component를 probe하지는 않습니다.
특히 `/metrics`는 애플리케이션에서 인증하지 않으므로 이 endpoint를 trusted network
안에 두세요.

Prometheus 호환 metric은 다음과 같이 조회합니다.
...
| 메트릭 | 타입 | 설명 |
|--------|------|-------------|
| `byoridb_query_total` | Counter | 성공적으로 완료된 쿼리 수 |
| `byoridb_query_latency_seconds` | Histogram | 실행이 반환된 뒤 기록한 쿼리 지연 시간 |
| `byoridb_query_errors_total` | Counter | query 실행 경로가 반환한 오류 수 |
| `byoridb_slow_queries_total` | Counter | 고정된 1초 threshold를 넘은 성공 쿼리 수 |
...
## 실행 중 query diagnostics

HTTP session을 만든 뒤 live GOD 또는 ADMIN session ID를 header에 전달합니다.
...
응답은 raw query text와 session ID를 제외합니다. Session header가 없거나 잘못됐거나
만료됐으면 HTTP 401, 유효하지만 admin이 아니면 HTTP 403을 반환합니다. Diagnostics
상태는 프로세스 안에만 있으며 재시작하면 사라집니다.
