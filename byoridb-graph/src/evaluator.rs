...
impl Evaluator {
        ...
        match expr {
            ...
            Expression::Identifier(name) => {
                ...
                ctx.get_property(name).ok_or_else(|| {
                    GraphError::ExecutionError(format!("Unknown identifier: {}", name))
                })
            }
            ...
            Expression::DstVertexProp { tag, prop } => Err(GraphError::ExecutionError(format!(
                "$$.{}.{} requires async context; use GO YIELD",
                tag, prop
            ))),
        }
    }
    ...
    fn eval_function(name: &str, args: &[Expression], ctx: &EvalContext) -> Result<Value> {
        ...
        match name.to_uppercase().as_str() {
            ...
            "LOWER" | "TOLOWER" => match args.first() {
                ...
                _ => Err(GraphError::ExecutionError(
            ...
            "IS_NULL" | "ISNULL" => match args.first() {
                ...
                None => Err(GraphError::ExecutionError(
    ...
    fn eval_add(left: Value, right: Value) -> Result<Value> {
        match (left, right) {
            ...
            (Value::Null(_), _) | (_, Value::Null(_)) => Ok(Value::null()),
            (l, r) => Err(GraphError::ExecutionError(format!(
                "Cannot add {:?} and {:?}",
                l, r
            ))),
        }
    ...
    fn eval_div(left: Value, right: Value) -> Result<Value> {
        match (left, right) {
            (_, Value::Int(0)) => Err(GraphError::ExecutionError("Division by zero".to_string())),
            (_, Value::Float(0.0)) => {
                Err(GraphError::ExecutionError("Division by zero".to_string()))
            }
            (Value::Int(l), Value::Int(r)) => Ok(Value::Int(l / r)),
            (Value::Float(l), Value::Float(r)) => Ok(Value::Float(l / r)),
            (Value::Int(l), Value::Float(r)) => Ok(Value::Float(l as f64 / r)),
    ...
    fn eval_mod(left: Value, right: Value) -> Result<Value> {
        match (left, right) {
            (_, Value::Int(0)) => Err(GraphError::ExecutionError("Modulo by zero".to_string())),
            (Value::Int(l), Value::Int(r)) => Ok(Value::Int(l % r)),
            (Value::Null(_), _) | (_, Value::Null(_)) => Ok(Value::null()),
            (l, r) => Err(GraphError::ExecutionError(format!(
                "Cannot modulo {:?} by {:?}",
...
mod tests {
    ...
    fn test_literal_evaluation() {
        ...
        assert_eq!(
            Evaluator::evaluate(&Expression::Literal(Literal::String("hello".to_string())))
                .unwrap(),
    ...
    fn test_comparison_operators() {
        ...
        let expr = Expression::BinaryOp {
            ...
            left: Box::new(Expression::Literal(Literal::Int(30))),
            right: Box::new(Expression::Literal(Literal::Int(20))),
        };
        assert_eq!(Evaluator::evaluate(&expr).unwrap(), Value::Bool(true));
    ...
    fn test_logical_operators() {
        ...
        let expr = Expression::UnaryOp {
            ...
            operand: Box::new(Expression::Literal(Literal::Bool(false))),
        };
        assert_eq!(Evaluator::evaluate(&expr).unwrap(), Value::Bool(true));
    }
    ...
    fn test_arithmetic_operators() {
        ...
        assert_eq!(Evaluator::evaluate(&expr).unwrap(), Value::Int(15));
        ...
        assert_eq!(Evaluator::evaluate(&expr).unwrap(), Value::Int(7));

        // 6 * 7
        let expr = Expression::BinaryOp {
            op: BinaryOperator::Mul,
            left: Box::new(Expression::Literal(Literal::Int(6))),
            right: Box::new(Expression::Literal(Literal::Int(7))),
        };
        assert_eq!(Evaluator::evaluate(&expr).unwrap(), Value::Int(42));
        ...
        assert_eq!(Evaluator::evaluate(&expr).unwrap(), Value::Int(5));

        // 17 % 5
        let expr = Expression::BinaryOp {
            op: BinaryOperator::Mod,
            left: Box::new(Expression::Literal(Literal::Int(17))),
            right: Box::new(Expression::Literal(Literal::Int(5))),
        };
        assert_eq!(Evaluator::evaluate(&expr).unwrap(), Value::Int(2));
    }
