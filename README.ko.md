...
> **Rust로 작성된 시맨틱 그래프 데이터베이스 — 온톨로지 추론, provenance,
> 시점 이력 조회를 제공합니다.**
...
standalone binary는 한 프로세스에 세 logical service를 실행합니다.

- **Graph service** (`byoridb-graph`): 쿼리를 parse, authorize, plan하고 실행을
  조정합니다. 인증 및 session state는 서버 실행 중 해당 프로세스에 존재합니다.
- **Meta service** (`byoridb-meta`): space, schema, partition 및 관련 metadata를
  관리합니다.
...
standalone 서버는 `BYORIDB_ROOT_PASSWORD`가 비어 있거나 설정되지 않으면 시작을
거부합니다. 배포 환경에서는 secret manager를, 개발 환경에서는 강력한 로컬
secret을 사용하세요.
