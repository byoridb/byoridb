...
[English (default)](QUICKSTART.md) | [한국어](QUICKSTART.ko.md)
...
[v0.3.3](https://github.com/byoridb/byoridb/releases/tag/v0.3.3), predates the
current `main` branch's authentication hardening, shared HTTP/gRPC session
state, temporal v1.1 changes, and edge `AS OF` reads. The commands below build
the current checkout; pin a commit SHA when reproducibility matters.
...
`byoridb-server` fails closed when `BYORIDB_ROOT_PASSWORD` is missing or empty.
It does not print or generate a retrievable root password.
...
Active-query diagnostics require a live `GOD` or `ADMIN` session in the
`x-byoridb-session-id` header:
...
The session ID stays out of the URL and response body. Treat the header as a
bearer credential and configure proxies and application logs to redact it.

## 7. Understand users and roles

`root` receives the `GOD` role. User and role administration, `SHOW USERS`,
`SHOW ROLES`, `SHOW SESSIONS`, and `BALANCE` require `GOD` or `ADMIN`. For
example:
...
The administrative `SHOW USERS` command (with legacy alias `SHOW USER`) lists
the built-in root account and persisted users with their roles. `SHOW ROLES`
(alias `SHOW ROLE`) currently returns that same user/role listing rather than a
separate role catalog. `SHOW SESSIONS` lists active users and selected spaces
but deliberately omits bearer session IDs.
